...
def main():
    # main logic to solve problem 3 with link http://www.pythonchallenge.com/pc/def/equality.html
    import re
    pattern = r"[^A-Z][A-Z][A-Z][A-Z]([a-z])[A-Z][A-Z][A-Z][^A-Z]"
    print "".join(re.findall(pattern, src))

def main2():
    # no re
    str_len = len(src)
    out = ""
    for x in range(4, str_len-3):
        cur = src[x]
        if cur.islower() and src[x-1].isupper() and src[x-2].isupper() and src[x-3].isupper() \
                and src[x+1].isupper() and src[x+2].isupper() and src[x+3].isupper() \
                and not src[x-4].isupper() and not src[x+4].isupper():
            out += src[x]
    print out
